...
def lambda_handler(event, context):
    for account in account_IDs:
        policies_to_add = [iam.Policy(arn='arn:aws:iam::' + account + ':policy/parameter_policy_A')]
        ...
        # Function for attaching policies on Roles, on Amazon Web Services IAM service (except roles defined in 'roles_not_to_attach_policies')
        ...
        for roles in response_iterator:
            for role in roles['Roles']:
                ...
                # Check if role name starts with letters from List of exceptions 'roles_not_to_attach_policies'
                if role_name.startswith(roles_not_to_attach_policies):
                ...
                # Attach policies, send 'error' message or 'policies_attached_to_role' message on AWS SQS service
                try:
                    for policy in policies_to_add:
                        print("Checking %s in role %r." % (policy, role_name))
                        if policy not in policies_on_role:
                            print("Policy %s is not in role %r, attaching..." % (policy, role_name))
                            ...
                            print("Policy %s is already in role %r." % (policy, role_name))
                        queue.send_message(MessageBody='Message: Role %r - list of policies: %s' % (
                            role_name, policy_names), MessageAttributes={
                            'Role': {
                                'StringValue': role_arn,
                                'DataType': 'String'
        ...
        # Function for attaching policies on Users, on Amazon Web Services IAM service (except users defined in 'users_not_to_attach_policies')
        ...
        for users in response_iterator:
            for user in users['Users']:
                ...
                if user_name.startswith(users_not_to_attach_policies):
                ...
                # Attach policies, send 'error' message or 'policies_attached_to_user' message on AWS SQS service
                try:
                    for policy in policies_to_add:
                        print("Checking %s in user %r." % (policy, user_name))
                        if policy not in policies_on_user:
                            print("Policy %s is not in user %r, attaching..." % (policy, user_name))
                            ...
                            print("Policy %s is already in user %r." % (policy, user_name))
                        queue.send_message(MessageBody='Message: User %r -  list of policies: %s' % (
                            user_name, policy_names), MessageAttributes={
                            'User': {
                                'StringValue': user_arn,
                                'DataType': 'String'
